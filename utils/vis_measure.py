...
for i, m in enumerate([5, 20, 100]):
    ...
    for worker in [1, 2, 4, 8]:
        data.append(np.load(os.path.join('logs/basis00/', 'time'+str(worker)+'_'+str(8)+'_0.0001_'+str(m)+'.npy')))
    # ax[0].plot(xticks, data, label='K={}'.format(h), color=color[i], marker=marker[i])
    ax[0].plot(xticks, data, label='M={}'.format(m), color=color[i], marker=marker[i])
...
for i, m in enumerate([5, 20, 100]):
    ...
    for worker in [1, 2, 4, 8]:
        ...
        for subworker in range(worker):
            name = 'acc_train_'+str(worker)+'_'+str(8)+'_0.0001_'+str(m)+'_'+str(subworker)+'.txt'
    ...
    ax[1].plot(xticks, data, label='M={}'.format(m), color=color[i], marker=marker[i])
...
for i, m in enumerate([5, 20, 100]):
    ...
    acc_train = []
    for subworker in range(worker):
        name = 'loss_'+str(worker)+'_'+str(8)+'_0.0001_'+str(m)+'_'+str(subworker)+'.npy'
        acc_train.append(np.load(os.path.join('logs/basis00/', name)))
    data = np.mean(acc_train, axis=0)[::10]
    ax[2].plot(x[::10], data, label='M={}'.format(m), color=color[i], marker=marker[i])
