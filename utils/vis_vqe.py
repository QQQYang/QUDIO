...
# ax.plot(xticks, xticks, color='k', label='linear speedup', linewidth=2.5)
...
#     ax.plot(xticks, data, label='W={}'.format(h), color=np.array(color[i])/255, marker=marker[i], linewidth=2.5, markersize=12)
...
# ax.set_ylabel('Speedup to time', font, fontsize=20)
# leg = ax.legend(loc='best', fontsize=20, frameon=True)
...
for i, h in enumerate([1, 2, 4, 8]):
    ...
    ax.errorbar([0.3, 0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 1.7, 1.9, 2.1], energy, yerr=energy_std, label='VQE: W={}'.format(h), color=np.array(color[i])/255, marker=marker[i], capsize=4, linewidth=2.5, markersize=12)
...
plt.savefig('figure/energy_surface_H2.png')
plt.show()
