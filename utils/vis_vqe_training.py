...
fig, ax = plt.subplots(1, 3, sharey=True, figsize=(14.4, 4.5)) # (12.8, 5.5)
title = ['(a)', '(b)', '(c)']
...
for j, worker in enumerate([2, 4, 8]):
    for i, h in enumerate([1, 2, 4, 8]):
        ...
        line, = ax[j].plot(x[1:200:8], data, label='W={}'.format(h), color=np.array(color[i])/255, marker=marker[i], markersize=6, linewidth=2.5)
        if j==0:
            lines.append(line)
            lines_label.append('W={}'.format(h))
    ax[j].plot(x[1:200:8], [-0.60180371]*len(data), '--', color='k')#, label='ground truth')
    if j==0:
        ax[j].set_ylabel('Ground state energy (Ha)', font, fontsize=20)
    # ax[1].set_ylim(0.8, 1.0)
    ax[j].set_xlabel('Iteration', font, fontsize=20)
    ax[j].set_ylim([-1.0, 2.2])
    # ax[j].set_title(title[j]+' Q={}'.format(worker), fontsize=20)
    ax[j].set_title('Q = {}'.format(worker), font, fontsize=20, y=0.8)
    ax[j].tick_params(labelsize=20)
    ax[j].grid(True)
    # leg=ax[j].legend(loc='best', fontsize=20, frameon=True)
    # leg.get_frame().set_linewidth(2)
    # leg.get_frame().set_edgecolor('k')
leg = fig.legend(lines, lines_label, loc=(0.87, 0.5), fontsize=20, frameon=True)
leg.get_frame().set_linewidth(2)
leg.get_frame().set_edgecolor('k')
...
plt.savefig('figure/app_vqe_training_loss.png')
